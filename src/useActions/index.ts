...
type ActionsInterface<R extends ReducersInterface> = {
  [K in keyof R]: PayloadType<R[K]> extends undefined
    ? () => void
    : (arg0: PayloadType<R[K]>) => void;
};

type MergeTypes<S, A> = [S, A];

export type StateAndActionsInterface<
  S,
  R extends ReducersInterface
> = MergeTypes<S, ActionsInterface<R>>;

export default function useActions<
  S,
  R extends ReducersInterface = ReducersInterface<S>
>(reducers: R, initialState: S): StateAndActionsInterface<S, R> {
  ...
  const actions = useMemo(
    () =>
      Object.keys(reducers).reduce((acs, k) => {
        return {
          ...acs,
          [k]: (payload) => {
            dispatch({ type: k, payload });
          },
        };
      }, initActions),
    [dispatch, reducers]
